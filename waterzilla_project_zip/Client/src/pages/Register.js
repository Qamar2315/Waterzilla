...
function Register() {
  ...
  const onSubmit = (data_, onSubmitProps) => {
    ...
    fetch("https://api.cloudinary.com/v1_1/dmiqkr7ja/image/upload", {
      ...
      .then(() => {
        axios.post(`http://localhost:8080/admin/dashboard/customers/register`, data_).then(() => {
          navigate("/login")
        })
      })
  };
  return (
    <>
      <div className='bg-slate-800 rounded mt-2 shadow-lg shadow-slate-900 pb-10 pt-2 pl-4 pr-4'>
        <div className='flex'>
          ...
          <Link to={'/home'}><h1 className='text-3xl mt-10 ml-5'>Waterzilla</h1></Link>
        </div>
        <div>
          <h1 className='text-xl text-zinc-200 mt-5'>Register yourself</h1>
          <Formik initialValues={initialValues} onSubmit={onSubmit} validationSchema={validationSchema}>
            <Form>
              ...
              <button className='btn btn-outline btn-sm btn-success mt-10' type='submit'>Register</button>
            </Form>
          </Formik>
        </div>
